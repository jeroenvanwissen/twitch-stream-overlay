...
export const chatShowDuration = ref(12);
export const chatAnimationDuration = ref(0.5);
...
export const texts = ref(["jeroenvanwissen"]);
...
export const open = async () => {
  ...
  const width = window.getComputedStyle(
    span.value as unknown as HTMLDivElement,
  ).width;
  (extension.value as unknown as HTMLDivElement).style.setProperty(
    "--card-width",
    `calc(${width})`,
  );
...
export const close = async (skip = false): Promise<void> => {
  ...
  return new Promise((resolve, reject): void => {
    ...
    setTimeout(() => {
      (extension.value as unknown as HTMLDivElement)!.classList.remove(
        "active",
      );
    }, 800);
...
export const cycle = async () => {
  ...
  (span.value as unknown as HTMLSpanElement).innerText =
    texts.value[index % texts.value.length]!;
...
export const roulette = async () => {
  interval = setInterval(
    ...
    (badgeOpenDuration.value + badgeClosedDuration.value) * 1000 +
      badgeTransitionDuration.value * 1000 * 2,
  );
...
watch(extension, async (value) => {
  ...
  (value as unknown as HTMLDivElement).style.transitionDuration =
    `${badgeTransitionDuration.value * 1000}ms`;
...
setInterval(
  () => {
    latestSubscriber.value &&
      messageNow(`Latest subscriber: ${latestSubscriber.value.displayName}`);
    latestFollower.value &&
      messageNow(`Latest follower: ${latestFollower.value?.displayName}`);
  },
  5 * 60 * 1000,
);
