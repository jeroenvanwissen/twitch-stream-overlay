...
const pronouns = ref<{ [key: string]: Pronoun }>(
  <{ [key: string]: Pronoun }>{},
);

(async () => {
  pronouns.value = await fetch(
    "https://api.pronouns.alejo.io/v1/pronouns",
  ).then((res) => res.json());
})();

export const getUserData = async (channel: string, id: string) => {
  const key = `${channel}-${id}`;
  if (users.has(key)) {
    const user = users.get(key);
    if (user && Date.now() - user.date < 15000) return user;
  }
  ...
  const pronoun = await fetch(
    `https://api.pronouns.alejo.io/v1/users/${userData.name}`,
  )
    .then((res) => res.json())
    .then((res) => {
