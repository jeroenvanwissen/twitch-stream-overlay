...
TwitchClient.botApiClient!.users.getUserByName(
  import.meta.env.VITE_TWITCH_BOT_NAME,
).then((user) => {
  console.log(user);
...
bot.onSubGift(async ({ broadcasterName, gifterName, userName }) => {
  await bot.say(
    broadcasterName,
    `Thanks @${gifterName} for gifting a subscription to @${userName}!`,
  );
});
...
bot.onBan(async ({ broadcasterName, userName }) => {
  await bot.say(
    broadcasterName,
    `Trash taken out! @${userName} has been banned!`,
  );
});
...
bot.onRaid(async ({ broadcasterName, userName, viewerCount }) => {
  await bot.announce(
    broadcasterName,
    `Thanks @${userName} for raiding with ${viewerCount} viewers!`,
    "primary",
  );
...
bot.onSub(async ({ broadcasterName, userName }) => {
  await bot.say(
    broadcasterName,
    `Thanks @${userName} for subscribing to the channel!`,
  );
...
bot.onResub(async ({ broadcasterName, userName, months }) => {
  await bot.say(
    broadcasterName,
    `Thanks @${userName} for subscribing to the channel for a total of ${months} months!`,
  );
});
...
// });
bot.onCommunityPayForward(
  async ({ broadcasterName, gifterDisplayName, originalGifterDisplayName }) => {
    await bot.say(
      broadcasterName,
...
bot.onGiftPaidUpgrade(
  async ({ broadcasterName, gifterDisplayName, userName }) => {
    await bot.say(
      broadcasterName,
      `Thanks @${userName} for continuing the gift sub gifted by ${gifterDisplayName}!`,
    );
  },
);
