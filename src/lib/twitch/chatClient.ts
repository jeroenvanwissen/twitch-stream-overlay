...
import {ref} from "vue";
import {getChannelBadges, getChannelFollowers, shoutoutUser} from "@/lib/twitch/apiClient";
import {botAuthProvider, getUserIdFromName} from "@/lib/twitch/bot";
...
messageHandler.value = chatClient.onMessage(async (channel: string, user: string, text: string, msg: ChatMessage) => {
    ...
    if (userData.badges.length == 0) {
        ...
        userData.badges = msg.userInfo.badges.size > 0
            ? [...msg.userInfo.badges.entries()]
                .map(([key, value]) => {
...
const handleCommand = async ({channel, command, params, newMsg}: {channel:string, command: string, params: string[], newMsg: Message}) => {
    ...
    switch (command) {
        case 'so':
            ...
            break;
        case 'followage':
            getChannelFollowers(broadcasterId, newMsg.userInfo.userId)
                .then(async ({data: [follow]}) => {
                    if (follow) {
                        const currentTimestamp = Date.now();
                        const followStartTimestamp = follow.followDate.getTime();
                        await chatClient.say(channel, `@${newMsg.userInfo.displayName} You have been following for ${secondsToDuration((currentTimestamp - followStartTimestamp) / 1000)}!`);
                    }
                    else {
