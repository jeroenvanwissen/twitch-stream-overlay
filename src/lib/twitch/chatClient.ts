...
const handleReward = async (channel: string, user: string, text: string, message: Message) => {
...
const handleCommand = async (channel: string, user: string, text: string, message: Message) => {
    const broadcasterId = (await getUserIdFromName(channel))!;
