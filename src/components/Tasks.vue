...
function startScrollAnimation() {
	if (!containerRef.value || !firstBlockRef.value || !secondBlockRef.value)
		return;
...
function checkOverflow() {
	if (!containerRef.value || !firstBlockRef.value)
		return;
...
<template>
	<div class="w-[380px] h-[600px] fixed left-[40px] bottom-[40px] p-4 overflow-hidden bg-theme-900/90 rounded-lg">
		<div class="mb-4">
			<h2 class="text-theme-300 text-2xl font-bold mb-2">
				Tasks
			</h2>
			<p class="text-theme-100 text-lg">
				!task [text], !focus #, !next #, !done #
			</p>
		</div>
		<div class="h-[calc(100%-4.5rem)] relative overflow-hidden py-2">
			<div v-if="hasTasks" ref="containerRef" class="tasks-container">
				<div ref="firstBlockRef" class="content-block">
						...
						<div class="mb-4 p-4 bg-theme-900/50 rounded-lg">
							...
							<ul>
								<li
									...
									:key="task.id"
									class="p-1 text-lg" :class="[task.done ? 'line-through' : '', task.focused ? 'text-theme-300' : '']"
								>
				...
				<div ref="secondBlockRef" class="content-block clone">
